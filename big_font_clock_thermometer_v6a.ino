...
v.5a - added adjust for temperature and hysteresis plusEEPROM store
v.5b - replaced lcd.print(String(char(223))); with lcd.write(223);
v.6 - replaced DS18B20 sensor with SHT31/AHT31 sensor: https://qqtrading.com.my/GY-SHT31-D
v.6a - control of brigthness via PWM, based on https://garrysblog.com/2022/06/30/dimming-16x2-and-20x4-lcd-displays/
*/
...
// Data wire for DS18B20 is connected to pin 6
...
#define back 11   // control PWM of backlight
...
int lumina = 127;    // pwm (max.255)
int lumina0;
byte pwmmin = 10;
byte pwmpas = 15;

unsigned long tprotire;
unsigned long tpsalvare = 10000;  // save pwm after (last) 10s from changed brigthness (pwm)
...
void setup () {
    ...
    pinMode(back, OUTPUT); // backlight 
    analogWrite(back, lumina);
...
if ((lumina < pwmmin) or (lumina > 255))
{
 EEPROM.write(603,127); // PWM value
 lumina = EEPROM.read(603);
}
lumina0 = lumina;
...
*/
   ...
   intt = tempC;
   inth = sht31.readHumidity();

  // Check if any reads failed and exit early (to try again).
  if (isnan(inth) || isnan(intt)) 
   {
    Serial.println(F("Failed to read from 1st sensor!"));
    lcd.setCursor(16, 2);
    lcd.print("FAIL");
    intsht = false;
   }
...
if (tempC >= te)  // ok
   ...
   // digitalWrite(relay, LOW);
    ...
    lumina0 = lumina;  // 
    if (millis() - tprotire > tpsalvare)
    {
     EEPROM.put(603, lumina); 
    }
...
void setclock (){
  ...
  // EEPROM.write(602,dete1); // 5/10 = 0.5 degree Celsius
   EEPROM.put(601,te1); // 50/2 = 25 degree Celsius
   EEPROM.put(602,dete1); // 5/10 = 0.5 degree Celsius
